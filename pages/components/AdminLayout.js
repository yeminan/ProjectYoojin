...
export default function AdminLayout({ children }) {
  ...
  const [userName, setUserName] = useState(""); // 로그인된 사용자 이름 저장
  ...
  fetch(`${ApiUrlKey}/api/users/check-auth`, {
    ...
    .then((data) => {
      if (!data.loggedIn) {
        ...
        setUserName(data.name || "관리자"); // ✅ 사용자 이름 저장
      }
    })
    .catch(() => {
      setIsLoggedIn(false);
      ...
      <Row>
        ...
        <Col md={10} className="p-4">
          ...
          <div className="d-flex justify-content-between align-items-center p-3 mb-3 rounded" style={{ backgroundColor: "#f8f9fa", boxShadow: "0px 2px 5px rgba(0, 0, 0, 0.1)" }}>
            ...
            <Nav.Link
              href="/admin/licenses"
              className="fw-bold fs-5 px-3 py-2 rounded text-white"
              style={{ backgroundColor: "#0d6efd", textDecoration: "none" }}
            >
              라이선스 현황
            </Nav.Link>
            {/* 회원가입 / 로그인 / 로그아웃 버튼 */}
